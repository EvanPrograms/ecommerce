...
const resolvers = {
  Query: {
    ...
    getProduct: async ({ productId }) => Product.findByPk(productId),
    ...
    getOrderHistory: async (_, __, { currentUser }) => {
      ...
      return orders.map(async (order) => {
        console.log("Raw shippingAddress from DB:", order.shippingAddress); 
  ...
  Mutation: {
    ...
    requestPasswordReset: async (_, { email }) => {
      ...
      const resetURL = `${DOMAIN}/resetpasswordform?userId=${user.id}&token=${token}`;      
